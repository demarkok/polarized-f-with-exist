...
The soundness proof can be viewed as a mapping from an algorithmic tree to a
...
\ruleref{\ottdruleATAppLetLabel}. To prove the soundness, 
in other words, to transform this rule into its
declarative counterpart \ruleref{\ottdruleDTAppLetLabel}, one needs
...
The only way to do so is by applying the soundness of singularity (\cref{sec:singularity-proo}), however, first, the declarative tree must be converted to the algorithmic one (by completeness!).
...
To deal with this issue, we introduce the metric on the entire 
\emph{inference trees} rather than on judgments, and plug into this metric the
parameter that certainly decreases in rules \ruleref{\ottdruleDTNEquivLabel} and \ruleref{\ottdruleDTPEquivLabel}---the number of these rules in the inference tree. We
