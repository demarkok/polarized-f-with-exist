...
The soundness proof requires completeness in the case of 
\ruleref{\ottdruleATAppLetLabel}, whose declarative counterpart requires \emph{uniqueness}
of the inferred application type. To show the uniqueness, we
need to conclude that any other \emph{declarative} tree 
must infer the same (or rather equivalent) type. 
The only way to do so is by applying the soundness of singularity (\cref{sec:singularity-proo}),
however, first, the declarative tree must be converted to the algorithmic one (by completeness!).

This way, these lemmas are proven by mutual induction. 
Since the declarative and the algorithmic systems are not mutually inductive,
we must introduce a metric on which the induction is conducted.
...
\begin{definition}[Number of prenex quantifiers]
    We define $\npq{[[iN]]}$ and $\npq{[[iP]]}$ as the number of 
    prenex quantifiers, i.e.
    \begin{itemize}
...
\begin{definition}[Declarative Tree Metric]
    \label{def:decl-typing-metric}
    For a tree $T$ inferring
    a declarative typing judgment $J$,
    we define a metric $\metric{T}$
...
\begin{definition}[Algorithmic Tree Metric]
