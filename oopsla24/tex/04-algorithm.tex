...
We define $[[dom(SC)]]$---the domain of constraint set $[[SC]]$---as a set of algorithmic variables
that it restricts. Similarly, we define $[[dom(Θ)]]$---the domain of instantiation context
---as a set of algorithmic variables that $[[Θ]]$ associates with their contexts.
We write $[[Θ(α̂±)]]$ to denote the declarative context associated with $[[α̂±]]$ in $[[Θ]]$.
...
\label{sec:subtyping-algorithm}
  ...
  as the normalization is preserved throughout the algorithm. However, we
