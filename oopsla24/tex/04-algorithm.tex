...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \node[below=0.8cm of 2] (7) {Constraint Singularity\\$[[SC singular with uσ]]$\\(\cref{sec:constraint-singularity})};
    \node[right=0.3cm of 7] (4) {Constraint Merge\\$[[Θ ⊢ SC1 & SC2 = SC3]]$\\(\cref{sec:constraint-merge})};
    \node[below=0.3cm of 7] (5) {Positive Subtyping\\$[[Γ ; Θ ⊨ uP1 ≥ iP2 ⫤ SC]]$\\(\cref{sec:subtyping-algorithm})};
...
The singularity algorithm is used to check that the inferred type of an
application is unique. In this case, the annotation in the applicative let
binder can be omitted. In the algorithmic system, the uniqueness means that the
\emph{constraint set} produced by the application inference has a single
substitution satisfying it.
