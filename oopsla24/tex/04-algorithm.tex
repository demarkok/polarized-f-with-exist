...
In addition to the rules repeating the declarative definition \cref{todo}, 
we have two base cases for the algorithmic variables:
 \ruleref{\ottdruleWFATPUVarLabel} and \ruleref{\ottdruleWFATNUVarLabel} (see \cref{fig:algo-wf})
...
We address this problem by algorithmization (\cref{todo})
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \node[right=1.4cm of 2] (3) {Application Inference\\$[[Γ ; Φ ; Θ1 ⊨ uN ● args ⇒> uM ⫤ Θ2 ; SC]]$};
    \node[below of=2,xshift=+2cm] (4) {Constraint Merge\\$[[Θ ⊢ SC1 & SC2 = SC3]]$\\(\cref{sec:constraint-merge})};
    \node[below of=4] (5) {Positive Subtyping\\$[[Γ ; Θ ⊨ uP1 ≥ iP2 ⫤ SC]]$\\(\cref{sec:subtyping-algorithm})};
    \node[left=0.75cm of 4] (6) {Negative Subtyping\\$[[Γ ; Θ ⊨ uN1 ≤ iN2 ⫤ SC]]$\\(\cref{sec:subtyping-algorithm})};
    ...
    \draw[->] (2) to (4);
    \draw[->] (3) to (4);
    \draw[->, bend right] (1.south west) to (5);
