...
\begin{proof}
  ...
  \item $[[Γ ⊨ ↓iN ∨ ↓iM = ∃nas.[nas / ToList Ξ]uP]]$
    ...
    To show $[[Γ ⊢ ∃nas.[nas / ToList Ξ]uP]]$,
    notice that $[[Γ, nas ⊢ [nas / ToList Ξ]uP]]$, which
    follows from $[[Γ ; Ξ ⊢ uP]]$ by \cref{todo}.

    To show $[[Γ ⊢ ∃nas.[nas / ToList Ξ]uP ≥ ↓iN]]$,
    we apply \ruleref{ottdruleDOneExistsLabel}.
    Notice that $[[ausi]]$ corresponds to $[[iNs/nas]]$

    % $[[Γ ; · ⊢ ausi : Ξ]]$ \\
    % $[[ [nas][nas / ToList Ξ]uP ≥ ↓iN]]$
