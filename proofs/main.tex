...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    [>={Stealth[scale=2]},node distance=2.6cm,every node/.style={draw,rectangle},every text node part/.style={align=center}]
...
inference for our language.
The judgement $[[Γ ; Φ ⊢ v : iP]]$
means that under the type context $[[Γ]]$ and
the variable context $[[Φ]]$, for the value $[[v]]$,
type $[[iP]]$ is inferrable. The negative form of the judgement is symmetric.
The application inference judgement $[[ Γ ; Φ ⊢ iN ● args ⇒> iM ]]$ means that if
a head of type $[[iN]]$ is applied to list of values $[[args]]$,
