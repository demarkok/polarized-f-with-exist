...
Observe the consistent polarization pattern of types. Consider the type for
$[[double]]$, which in \systemf would be $[[Int → Int]]$. In polarized form, it
becomes $[[↓(Int → ↑Int)]]$---a \emph{suspended} function (indicated by the
downshift) that accepts an integer $[[Int]]$ and yields an integer
\emph{computation} (indicated by the upshift) $[[↑Int]]$. 

Moving on to the examples in \cref{fig:polarization-examples}, we will delve
into several cases that demonstrate the capabilities of type inference within
\fexists.
