...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    % Define nodes
    \node[xshift=-1cm] (1) {Negative Subtyping\\$[[Γ ; Θ ⊨ uN ≤ iM ⫤ SC]]$\\(\cref{fig:alg-subtyping})};
    \node[below of=1] (3) {Positive Subtyping\\$[[Γ ; Θ ⊨ uP ≥ iQ ⫤ SC]]$\\(\cref{fig:alg-subtyping})};
    \node[left=1.7cm of 3] (2) {Constraint Merge\\$[[Θ ⊢ SC1 & SC2 = SC3]]$\\(\cref{sec:constraint-merge})};
    ...
    \node[below of=6,xshift=.4cm] (7) {Anti-Unification\\$[[Γ ⊨ iP1 ≈au iP2 ⫤ ( Ξ , uQ , aus1 , aus2 )]]$\\$[[Γ ⊨ iN1 ≈au iN2 ⫤ ( Ξ , uM , aus1 , aus2 )]]$\\(\cref{sec:antiunification})};
    \node[below of=5] (8) {Unif. Constraint Merge\\$[[Θ ⊢ UC1 & UC2 = UC3]]$\\(\cref{sec:constraint-merge})};
    ...
    \draw[->] (4) to (6);
    \draw[->] (5) to (8);
    \draw[->] (6) to node[left, draw=none]{$\ottkw{nf}$} ($(7.north) + (-.4cm, 0)$);
  \end{tikzpicture}  
...
\paragraph*{The anti-unification rule}
  ...
  they fail, apply \ruleref{\ottdruleAUAULabel}.
...
\begin{figure}[t]
  \centering
  \begin{tikzpicture}
    [>={Stealth[scale=2]},node distance=2.1cm,every node/.style={draw,rectangle},every text node part/.style={align=center}]
    % Define nodes
    \node[] (1) {Positive Inference\\$[[Γ; Φ ⊨ v : iP]]$};
    \node[right=1.1cm of 1] (2) {Negative Inference\\$[[Γ; Φ ⊨ c : iN]]$};
    \node[right=0.8cm of 2] (3) {Application Inference\\$[[Γ ; Φ ; Θ1 ⊨ uN ● args ⇒> uM ⫤ Θ2 ; SC]]$};
    \node[below=0.6cm of 2, xshift=+0.4cm] (7) {Minimal Instantiation\\$[[Γ ⊢ uP SC minby uσ]]$\\(\cref{sec:constraint-singularity})};
    \node[below=0.5cm of 7] (8) {Constraint Singularity\\$[[SC singular with uσ]]$\\(\cref{sec:constraint-singularity})};
    \node[right=0.2cm of 7] (4) {Constraint Merge\\$[[Θ ⊢ SC1 & SC2 = SC3]]$\\(\cref{sec:constraint-merge})};
    \node[left=0.3cm of 7] (6) {Negative Subtyping\\$[[Γ ; Θ ⊨ uN1 ≤ iN2 ⫤ SC]]$\\(\cref{sec:subtyping-algorithm})};
    \node[below=0.9cm of 6, xshift=+.3cm] (5) {Positive Subtyping\\$[[Γ ; Θ ⊨ uP1 ≥ iP2 ⫤ SC]]$\\(\cref{sec:subtyping-algorithm})};
    \draw[->] ([yshift=-.2cm]2.west) to ([yshift=-.2cm]1.east);
    \draw[->] ([yshift=+.2cm]1.east) to ([yshift=+.2cm]2.west);
    \draw[->, bend right=20] (3) to (1);
    \draw[->] (2) to (3);
    \draw[->] (2) to ([xshift=+.2cm]4.north west);
    \draw[->] ([xshift=+.4cm]2.south) to (7);
    \draw[->] (3) to (4);
    \draw[->] (7) to (8);
    \draw[->] (1.south west) |- (5);
    \draw[->] ([xshift=+1.2cm]3.south) |- (5.south east);
    \draw[->] (2) to (6);
  \end{tikzpicture}  
...
The positive and the negative type inference judgments have the symmetric forms:
...
The application type inference judgment has the form 
